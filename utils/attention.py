...
def register_attention_editor_diffusers(model, editor: AttentionBase):
    ...
    def register_editor(net, count, place_in_unet, parent_name):
        for name, subnet in net.named_children():
            if net.__class__.__name__ == 'Attention':  # spatial Transformer layer
                # print(f"Registering {parent_name}.{name} with {place_in_unet} and {count}")
                net.forward = ca_forward(net, place_in_unet)
                return count + 1
            elif hasattr(net, 'children'):
                count = register_editor(subnet, count, place_in_unet, parent_name+"."+name)
        return count

    cross_att_count = 0
    for net_name, net in model.unet.named_children():
        if "down" in net_name:
            cross_att_count += register_editor(net, 0, "down", "unet")
        elif "mid" in net_name:
            cross_att_count += register_editor(net, 0, "mid", "unet")
        elif "up" in net_name:
            cross_att_count += register_editor(net, 0, "up", "unet")
